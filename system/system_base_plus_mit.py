...
class LightningSystem(system_abstract_moco.LightningSystem):
    ...
    def add_model_specific_args(parent_parser):
        parser = system_abstract_moco.LightningSystem.add_model_specific_args(
            parent_parser)
        parser.add_argument("--model_name", type=str, default="bplusmit")
        ...
        parser.add_argument("--intra_all",
                            default='true',
                            choices=['true', 'false'])
        parser.add_argument("--lm_moco", type=float, default=0.2)
    ...
    def forward(self, xq, xk, labels):
        out_enc, loss_sup = self.moco(xq, xk, labels)
        out_cls = self.base_head(out_enc)
        return loss_sup, out_cls
    ...
    def _training_step(self, batch, batch_idx):
        ...
        moco_loss, out_cls = self(xq, xk, label)
        ...
        with torch.no_grad():
            ...
            try:
                ...
                acc = torch.tensor(0., device=self.device, type=self.dtype)
        tqdm_dict = {
            "loss_train": loss,
            "moco_loss": moco_loss,
            "mlp_loss": mlp_loss,
