...
class LightningSystem(system_abstract_moco.LightningSystem):
    ...
    def add_model_specific_args(parent_parser):
        parser = system_abstract_moco.LightningSystem.add_model_specific_args(
            parent_parser)
        parser.add_argument("--model_name", type=str, default="bplusmtwo")
    ...
    def _training_step(self, batch, batch_idx):
        ...
        # x_ce = torch.cat((xq, xk), dim=0)
        label_ce = torch.cat((label, label), dim=0)
        output, target, out_cls = self(xq, xk, label)

        mlp_loss = F.cross_entropy(out_cls, label_ce)
...
class MoCo(nn.Module):
    ...
    def forward(self, im_q, im_k, labels):
        ...
        # im_qk = torch.cat((im_q, im_k), dim=0)
        # qk_enc = self.encoder_q[0](im_qk)  # queries: NxC

        k_enc = self.encoder_q[0](im_k)  # queries: NxC

        # q_enc = qk_enc[:im_q.shape[0]]
        ...
        return logits, labels_for_softmax, torch.cat((q_enc, k_enc), dim=0)
