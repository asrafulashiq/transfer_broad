    ...
    def _compute_block_mask(self, mask):
        ...
        offsets = torch.stack([
            ...
            torch.arange(self.block_size).repeat(
                self.block_size),  #- left_padding
        ]).t().cuda()
        offsets = torch.cat(
            (torch.zeros(self.block_size**2, 2).cuda().long(), offsets.long()),
            1)
    ...
    def _make_layer(self,
        ...
        if n_block == 1:
            layer = block(self.inplanes, planes, stride, downsample, drop_rate,
                          drop_block, block_size, self.use_se)
        else:
            layer = block(self.inplanes, planes, stride, downsample, drop_rate,
                          self.use_se)
        layers.append(layer)
...
def resnet18(keep_prob=1.0, avg_pool=False, **kwargs):
    ...
    """
    model = ResNet(BasicBlock, [1, 1, 2, 2],
                   keep_prob=keep_prob,
                   avg_pool=avg_pool,
                   **kwargs)
    return model
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model',
                        type=str,
                        choices=[
                            'resnet12', 'resnet18', 'resnet24', 'resnet50',
                            'resnet101', 'seresnet12', 'seresnet18',
                            'seresnet24', 'seresnet50', 'seresnet101'
                        ])
    ...
    model = model_dict[args.model](avg_pool=True,
                                   drop_rate=0.1,
                                   dropblock_size=5,
                                   num_classes=64)
